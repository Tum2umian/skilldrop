...
// Detect if we are inside the "auth" or "admin" directory
...
// Adjust base path if inside "auth" or "admin" folders
if ($is_auth || $is_admin || $is_worker) {
...
// Set the correct dashboard link based on the role
$dashboard_link = ($user_role === 'admin') ? $base_url . 'admin/dashboard.php' : $base_url . 'dashboard.php';

// Fetch user profile image (if logged in)
$profile_image = $is_logged_in && !empty($_SESSION['profile_image'])
    ? $base_url . $_SESSION['profile_image']
    : $base_url . 'uploads/profile_images/default.svg';

// Set profile edit link based on user role
$edit_profile_link = ($user_role === 'admin') ? $base_url . 'admin/edit-profile.php' : $base_url . 'edit-profile.php';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<li class="user-profile">
    <img src="<?php echo $profile_image; ?>" alt="Profile Picture" 
        class="profile-icon" id="profileIcon" onclick="toggleProfileMenu()">

    <div class="profile-dropdown" id="profileDropdown">
        <p>Logged in as: <strong><?php echo ucfirst($user_role); ?></strong></p>

        <?php
        // Define the dynamic dashboard and edit profile links based on user role
        if ($user_role === 'admin') {
            ...
            $dashboard_link = "#"; // Default case, in case role is undefined
            $edit_profile_link = "#";
        }
        ?>

        <a href="<?php echo $dashboard_link; ?>" class="dropdown-link"><i class="fas fa-tachometer-alt"></i> Dashboard</a>
        <a href="<?php echo $edit_profile_link; ?>" class="dropdown-link"><i class="fas fa-user-edit"></i> Edit Profile</a>
        <a href="<?php echo $base_url; ?>logout.php" class="dropdown-link"><i class="fas fa-sign-out-alt"></i> Logout</a>
    </div>
</li>
